...
@torch.no_grad()
def coarse_matching(query_view, map_view, model, device, pixel_tol, fast_nn_params, events):
    # prepare batch
    imgs = []
    ...
    output = inference([tuple(imgs)], model, device, batch_size=1, verbose=False, events=events)
    ...
    if pixel_tol == 0:
        start_fastnn.record()
        matches_im_map, matches_im_query = fast_reciprocal_NNs(PM, PQ, subsample_or_initxy1=8, events=events, **fast_nn_params)
        end_fastnn.record()
        ...
        start_fastnn.record()
        matches_im_map, matches_im_query = fast_reciprocal_NNs(PM, PQ, (xM, yM), pixel_tol=pixel_tol, events=events, **fast_nn_params)
        end_fastnn.record()
...
@torch.no_grad()
def crops_inference(pairs, model, device, batch_size=48, verbose=True, **kw):
    ...
    if B < batch_size:
        return loss_of_one_batch(pairs, model, None, device=device, symmetrize_batch=False, **kw)
    preds = []
    for ii in range(0, B, batch_size):
        ...
        preds.append(loss_of_one_batch(temp_data, model,
                                       None, device=device, symmetrize_batch=False, **kw))  # sequential forward
    # Merge all preds
    return cat_collate(preds, collate_fn_map=cat_collate_fn_map)


@torch.no_grad()
def fine_matching(query_views, map_views, model, device, max_batch_size, pixel_tol, fast_nn_params, events):
    assert pixel_tol > 0
    output = crops_inference([query_views, map_views],
                             model, device, batch_size=max_batch_size, verbose=False, events=events)
    pred1, pred2 = output['pred1'], output['pred2']
    ...
    for ppi, (pp1, pp2, cc11, cc21) in enumerate(zip(descs1, descs2, confs1, confs2)):
        ...
        matches_im_map_ppi, matches_im_query_ppi = fast_reciprocal_NNs(pp2, pp1, (x_ppi, y_ppi),
                                                                       pixel_tol=pixel_tol, events=events, **fast_nn_params)
        end_fastnn.record()
        events['start_fastnn'].append(start_fastnn)
        events['end_fastnn'].append(end_fastnn)
...
if __name__ == '__main__':
    ...
    fast_nn_params = dict(device=device, dist='dot', block_size=2**13, use_amp=args.use_amp, single_loop=args.single_loop)
