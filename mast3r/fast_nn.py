...
@torch.no_grad()
def bruteforce_reciprocal_nns(A, B, device='cuda', block_size=None, dist='l2', events=None, single_loop=False, use_amp=False):
    ...
    with autocast(enabled=use_amp):
        ...
        if events is not None:
            start_bnn = torch.cuda.Event(enable_timing=True)
            end_bnn = torch.cuda.Event(enable_timing=True)
            start_bnn.record()
        ...
        if block_size is None or len(A) * len(B) <= block_size**2:
            ...
            if single_loop:
                for i in range(number_of_iteration_A):
                    ...
                    line_mask = min_B_j < dis_B

                    dis_A[i * block_size:(i + 1) * block_size][col_mask] = min_A_i[col_mask].to(dis_A.dtype)
                    dis_B[line_mask] = min_B_j[line_mask].to(dis_B.dtype)

                    nn_A[i * block_size:(i + 1) * block_size][col_mask] = argmin_A_i[col_mask]
                    nn_B[line_mask] = argmin_B_j[line_mask] + (i * block_size)
                ...
                for i in range(number_of_iteration_A):
                    ...
                    for j in range(number_of_iteration_B):
                        ...
                        dis_A[i * block_size:(i + 1) * block_size][col_mask] = min_A_i[col_mask].to(dis_A.dtype)
                        dis_B[j * block_size:(j + 1) * block_size][line_mask] = min_B_j[line_mask].to(dis_B.dtype)
        ...
        if events is not None:
            end_bnn.record()
            events['start_bnn'].append(start_bnn)
            events['end_bnn'].append(end_bnn)
...
def fast_reciprocal_NNs(pts1, pts2, subsample_or_initxy1=8, ret_xy=True, pixel_tol=0, ret_basin=False,
    ...
    niter = 0
    # n_notyet = [len(notyet)]
    if 'single_loop' in matcher_kw and matcher_kw['single_loop'] == True:
        torch.set_float32_matmul_precision('highest')
    while notyet.any():
        with autocast(enabled=matcher_kw['use_amp'] if 'use_amp' in matcher_kw else False):
