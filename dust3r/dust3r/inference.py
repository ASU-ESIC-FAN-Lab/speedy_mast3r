...
def loss_of_one_batch(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None, **kw):
    view1, view2 = batch
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        pred1, pred2 = model(view1, view2, **kw)
...
@torch.no_grad()
def inference(pairs, model, device, batch_size=8, verbose=True, **kw):
    if verbose:
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        res = loss_of_one_batch(collate_with_cat(pairs[i:i + batch_size]), model, None, device, **kw)
