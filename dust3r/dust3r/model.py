...
class AsymmetricCroCo3DStereo (
    ...
    def forward(self, view1, view2, events=None):
        if events is not None:
            ...
            start_downstream_head = torch.cuda.Event(enable_timing=True)
            end_downstream_head = torch.cuda.Event(enable_timing=True)
            ...
            start_downstream_head.record()
            with torch.cuda.amp.autocast(enabled=False):
                res1 = self._downstream_head(1, [tok.float() for tok in dec1], shape1)
                res2 = self._downstream_head(2, [tok.float() for tok in dec2], shape2)
            end_downstream_head.record()

            events['start_encode'].append(start_encode)
            events['end_encode'].append(end_encode)
            events['start_decode'].append(start_decode)
            events['end_decode'].append(end_decode)
            events['start_downstream_head'].append(start_downstream_head)
            events['end_downstream_head'].append(end_downstream_head)
